...
RUNS = [
    ("Baseline CNN", "baseline_cnn_official_leafsafe_full"),
    ("ResNet18", "resnet18_official_leafsafe_full"),
    ("EfficientNet-B0", "efficientnet_b0_official_leafsafe_full"),
    (
        "ResNet18 + EfficientNet-B0 Ensemble",
        "resnet18_efficientnet_official_leafsafe_soft_voting",
    ),
]
...
def load_summary(run_name):
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing evaluation summary: {path}\nRun all official evaluations first."
        )
...
def main():
    ...
    for label, run_name in RUNS:
        ...
        if int(summary.get("test_examples", -1)) != 10709:
            raise RuntimeError(f"{run_name} does not use the 10,709-image locked test.")
        protocol = summary.get("data_protocol") or summary.get("evaluation_protocol")
        if protocol != "conservative_official_leaf_preserving_test":
            raise RuntimeError(f"{run_name} has an unexpected data protocol: {protocol}")
        ...
        rows.append(
            {
                ...
                "errors": int(summary.get("errors", round((1.0 - summary["test_accuracy"]) * summary["test_examples"]))),
                "summary_path": str(path.resolve()),
            }
        )

    frame = pd.DataFrame(rows)
    output_dir = EVALUATION_ROOT / "official_leafsafe_final_comparison"
    output_dir.mkdir(parents=True, exist_ok=True)
    FIGURE_ROOT.mkdir(parents=True, exist_ok=True)

    csv_path = output_dir / "final_model_comparison.csv"
    json_path = output_dir / "final_model_comparison.json"
    figure_path = FIGURE_ROOT / "official_leafsafe_final_model_comparison.png"
    ...
    axis.set_title("PlantVillage - Conservative Official Leaf-Safe Comparison")
    ...
    print("=" * 100)
    print("FINAL CONSERVATIVE OFFICIAL LEAF-SAFE RESULTS")
    print("=" * 100)
    display = frame[
        ["model", "accuracy", "macro_f1", "weighted_f1", "macro_roc_auc_ovr", "errors"]
    ].copy()
