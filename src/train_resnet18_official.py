...
from transfer_models import create_resnet18_transfer, split_resnet18_parameters
...
DEFAULT_CLASSIFIER_LR = 5e-4
DEFAULT_WEIGHT_DECAY = 1e-4
DEFAULT_DROPOUT = 0.3
EARLY_STOPPING_PATIENCE = 5
DEFAULT_RUN_NAME = "resnet18_official_leafsafe_full"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fine-tune ImageNet-pretrained ResNet18 on the conservative official PlantVillage leaf-safe split."
    )
    parser.add_argument("--epochs", type=int, default=DEFAULT_EPOCHS)
    parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH_SIZE)
    parser.add_argument("--num-workers", type=int, default=NUM_WORKERS)
    parser.add_argument("--backbone-learning-rate", type=float, default=DEFAULT_BACKBONE_LR)
    parser.add_argument("--classifier-learning-rate", type=float, default=DEFAULT_CLASSIFIER_LR)
...
def main():
    ...
    backbone_parameters, classifier_parameters = split_resnet18_parameters(model)
    ...
    config = {
        ...
        "training_strategy": "full_fine_tuning_with_differential_learning_rates",
    ...
    print("=" * 76)
    print("RESNET18 - CONSERVATIVE OFFICIAL LEAF-SAFE TRAINING")
    print("=" * 76)
    ...
    plot_training_history(
        history,
        curves_path,
        f"ResNet18 - Official Leaf-Safe - {args.run_name}",
    )
