...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            "Kaydedilmiş Baseline CNN modelini kilitli "
...
def main():
    ...
    if checkpoint.get("model_name") not in (
        ...
        raise ValueError(
            ...
            "baseline_cnn checkpoint'i kabul eder."
        )
    ...
    dropout_rate = float(
        checkpoint.get("dropout_rate", 0.4)
    )
    model = BaselineCNN(
    ...
    print("=" * 65)
    print("BASELINE CNN KİLİTLİ TEST DEĞERLENDİRMESİ")
    print("=" * 65)
