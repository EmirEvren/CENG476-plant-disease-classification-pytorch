...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate the baseline CNN on the locked conservative official PlantVillage test set."
    )
...
def main():
    ...
    if checkpoint.get("model_name") != "baseline_cnn":
        raise ValueError("Checkpoint is not a baseline_cnn model.")
    ...
    dropout = float(checkpoint.get("dropout_rate", 0.4))
    model = BaselineCNN(num_classes=len(class_names), dropout_rate=dropout).to(device)
    ...
    print("=" * 76)
    print("BASELINE CNN - LOCKED OFFICIAL LEAF-SAFE TEST")
    print("=" * 76)
